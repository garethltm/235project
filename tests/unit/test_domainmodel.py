...
def test_wishlist_add_game(wishlist, game):
    wishlist.add_game(game)
    assert len(wishlist.list_of_games()) == 1
    assert wishlist.list_of_games()[0] == game


def test_wishlist_remove_game(wishlist, game):
    wishlist.add_game(game)
    wishlist.remove_game(game)
    assert len(wishlist.list_of_games()) == 0


def test_wishlist_select_game(wishlist, game):
    wishlist.add_game(game)
    assert wishlist.select_game(0) == game


def test_wishlist_select_game_out_of_index(wishlist):
    assert wishlist.select_game(0) is None


def test_wishlist_first_game_in_list(wishlist, game):
    wishlist.add_game(game)
    assert wishlist.first_game_in_list() == game


def test_wishlist_first_game_in_empty_list(wishlist):
